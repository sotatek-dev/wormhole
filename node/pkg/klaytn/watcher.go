	...
	go func() {
		for {
			...
			case r := <-e.obsvReqC:
				...
				logger.Info("Block", zap.Uint64("number", blockNumberU))
				if blockNumberU == 0 {
					logger.Info("no block number available, ignoring observation request",
				...
				if err != nil {
					logger.Info("failed to process observation request",
